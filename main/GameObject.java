...
public class GameObject {
	...
	private void startGame() { // creates new everything
	    ...
	    int totalChests = 2000;
	    int mapWidth = map.WIDTH;
	    int mapHeight = map.HEIGHT;
	    int safetyDistance = 300; // Don't spawn chests within 300px of player start

	    for (int i = 0; i < totalChests; i++) {
	        int rx, ry;
	        
	        // Keep rolling random coordinates until we are outside the safety zone
	        do {
	            rx = (int) (Math.random() * mapWidth);
	            ry = (int) (Math.random() * mapHeight);
	        } while (Math.abs(rx - player.getX()) < safetyDistance && 
	                 Math.abs(ry - player.getY()) < safetyDistance);

	        getChests().add(new Chest(this, rx, ry));
	    }
	}
