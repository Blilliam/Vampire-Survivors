...
public class BouncingProjectile extends WeaponEntity {
    ...
    private void bounce(Enemy currentTarget) {
        ...
        for (Enemy e : gameObj.getEnemies()) {
            if (e == currentTarget || e.isDead()) continue;
        ...
        if (nextTarget != null) {
            Vec2 newDir = new Vec2(nextTarget.getX() - position.getX(), 
                                   nextTarget.getY() - position.getY());
            this.velocity = newDir.normalize().scale(weapon.getSpeed());
        }
    }
}